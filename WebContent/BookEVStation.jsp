          ...
          <li class="dropdown"><a href="#"><span>Welcome<%response.setContentType("text/html");  
				 HttpSession sessio=request.getSession(true);  
	               ...
	               String email=(String)session.getAttribute("email");  
	                out.print(" "+email+""); }   %></span> <i class="bi bi-chevron-down"></i></a>
            <ul>
...
<table class="table table-bordered" style="color:black">
          ...
          <br>
          <input type="text" name="slot" class="form-control" id="slot"   placeholder="slot" required>
          <br>
                ...
                try{
                   ...
                   /*  ResultSet rs=st.executeQuery("select addcategoryauction.id,addcategoryauction.categoryname,addsubcategoryauction.subcategoryname,addsessiondateauction.date,addsessiontimeauction.time,addcategoryauction.status from addcategoryauction inner join addsubcategoryauction on addcategoryauction.cat=addsubcategoryauction.cat inner join addsessiondateauction on addcategoryauction.cat=addsessiondateauction.cat inner join addsessiontimeauction on addcategoryauction.cat=addsessiontimeauction.cat");
                    ...
                    //while(rs.next())
                 	...
                 	Connection con = DBConnection.getConnection();
    Statement st=con.createStatement();
      ResultSet rs=st.executeQuery("select * from create_evstation where ownername='"+ownername+"'");  
 /*   ResultSet rs=st.executeQuery("select ower.id,ower.ownername,ower.bankname,ower.bankaddr,ower.accountno,ower.ifsc,create_evstation,ownername,ower.status from owner inner join create_evstation on owner.ownername=create_evstation.ownername");
    */            
while(rs.next()) {
                   	%>
         
         <input type="text" name="ownername" class="form-control" value="<%=rs.getString("ownername") %>"   placeholder="Account Holder Name" readonly>
          <br>
         
			<input type="text" name="accountname" class="form-control"   placeholder="Bank Name" required>
          <br>
           <input type="text" name="accountno" class="form-control"  placeholder="Bank Address" required>
           <br>
              <input type="text" name="ifsc" class="form-control" placeholder="Account No." required>
           <br>
           
           <input type="text" name="amount" class="form-control" placeholder="Amount" required>
           <br>
           <input type="text" name="mobileno" class="form-control" value="<%=rs.getString("mobileno") %>"  readonly>
          <br>
           <% 
                   	}
                }catch(Exception e)
             ...
             <button type="submit"  class="button" value="login" ><b>Book</b></button> 
			</div>
